...
class EditPrintFilesDrawer extends Component {
    ...
    UNSAFE_componentWillReceiveProps(nextProps) {
        ...
        if (
            nextProps.editPrintFilesError && nextProps.editPrintFilesError !== editPrintFilesError
        ) {
            message.error(CatchError[nextProps.editPrintFilesError] || nextProps.editPrintFilesError);
        }
    ...
    onFinish = (values) => {
        ...
        if (data.id) {
            let listIdImage = [];
            for (let i = 0; i < data.productTypes.length; i += 1) {
                for (let j = 0; j < data.productTypes[i].printFileImages.length; j += 1) {
                    if (data.productTypes[i].printFileImages[j].image && data.productTypes[i].printFileImages[j].image.id) {
                        listIdImage.push(data.productTypes[i].printFileImages[j].image.id)
                    }
                }
            }
            ...
            // console.log({productTypeUpdates: productTypeUpdates, productId: data.id})
            editPrintFiles({productTypeUpdates: productTypeUpdates, productId: data.id})
        }
    };

    render() {

        const {onClose, visible, data, editPrintFilesLoading} = this.props;
                                                                            ...
                                                                            <Row key={i}>
                                                                                <Col md={9}>
                                                                                    <Form.Item
                                                                                        ...
                                                                                        style={{
                                                                                            marginBottom: i !== fields2.length - 1 ? 20 : 0,
                                                                                            width: '100%'
                                                                                        }}
                                                                                        valuePropName="file"
                                                                                    ...
                                                                                    >
                                                                                        <Upload
                                                                                            disabled={currentPrintFile.custom}
                                                                                            className={classnames('productPrintFile-uploader', {
                                                                                                'warning': (currentPrintFile.custom && currentPrintFile.image) || !currentPrintFile.image
                                                                                            })}
                                                                                            {...props}
                                                                                        >
                                                                                            {
                                                                                                (currentPrintFile.custom && currentPrintFile.image) || !currentPrintFile.image ?
                                                                                                    uploadButton :
                                                                                                    <img
